...
for i in range(0, len(comp_artists)):
    if(comp_artists['2024'][i]in comp_artists['2023'].value_counts()):
        ...
        comp_artists['2024'][i]=CRED+"　　　　　　　　　"+comp_artists['2024'][i]+CEND
        comp_artists['2023'][ans]=CRED+"　　　　　　　　　"+comp_artists['2023'][ans]+CEND
...
if(option=="comparison"):
    ...
    if(obscurity-obscurity_comp<0):
        print("\n you liked", obscurity, "obscure songs,", abs(obscurity-obscurity_comp), "less than last year\n")
    else:
        print("\n you liked", obscurity, "obscure songs,", abs(obscurity-obscurity_comp), "more than last year\n")
        
    if(len(this_year)-len(last_year)<0):
        print("\n you liked",len(this_year), "songs in total,", abs(len(this_year)-len(last_year)), "less than last year\n")
    else:
    ...
    if(len(artists)-len(last_artists)<0):
        print("\n you liked songs by",len(artists),"artists,", abs(len(artists)-len(last_artists)), "less than last year\n")
    else:
